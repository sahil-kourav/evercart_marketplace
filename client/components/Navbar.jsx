...
//       >
//         <div className="max-w-7xl mx-auto px-4 h-[62px] flex  items-center gap-4">
...
//           {/* ----- Search feature is temporarily removed, will be added back in the future ----- */}
//           {/* <form
...
const DesktopUserDropdown = ({ user, onLogout }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((p) => !p)}
        className={`flex items-center gap-2 pl-1 pr-3 py-1 rounded-xl border transition-all duration-200
          ${open
            ? "bg-slate-100 border-slate-200"
            : "border-transparent hover:bg-slate-50 hover:border-slate-200"
          }`}
      >
        <Avatar name={displayName} size={28} />
        <span className="text-[13px] font-medium text-slate-700 max-w-[90px] truncate">
          {displayName.split(" ")[0]}
        </span>
        <svg
        ...
        <div className="p-2">
          {ACCOUNT_LINKS.map(({ label, href, icon, iconBg }) => (
            <Link
              ...
              className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-slate-50 transition-colors group"
            >
              <span className={`w-8 h-8 ${iconBg} rounded-lg flex items-center justify-center shrink-0`}>
                {icon}
              </span>
              <span className="text-[13px] font-medium text-slate-600 group-hover:text-indigo-600 transition-colors">
                {label}
...
const Navbar = () => {
  ...
  return (
    <>
      ...
      >
        {/* 3-column layout: logo | center | actions */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-5 h-[64px] grid grid-cols-[auto_1fr_auto] items-center">
          ...
          {/* ── Col 2: Nav links + search (centered) ── */}
          <div className="hidden md:flex items-center justify-center gap-1">
            {NAV_LINKS.map(({ label, href }) => (
              <Link
                key={label}
                href={href}
                className="text-[14px] font-medium text-slate-500 hover:text-slate-900
                  hover:bg-slate-50 px-4 py-2 rounded-lg transition-all duration-150 whitespace-nowrap"
              >
                {label}
              </Link>
            ))}

            {/* Search — inline after nav links on xl */}
            {/* ----- Search feature is temporarily removed, will be added back in the future ----- */}
            {/*
            <div className="hidden xl:flex items-center ml-3 pl-3 border-l border-slate-200">
              <form
                onSubmit={handleSearch}
                className="flex items-center gap-2 bg-slate-50 focus-within:bg-white
                  px-3 py-1.5 rounded-xl border border-slate-100 focus-within:border-slate-300
                  transition-all duration-200 focus-within:shadow-sm w-[220px] focus-within:w-[280px]"
                style={{ transition: "width 200ms ease, box-shadow 200ms ease, background 200ms ease, border-color 200ms ease" }}
              >
                <Search size={13} className="text-slate-400 shrink-0" />
                <input
                  className="bg-transparent outline-none flex-1 text-[13px] text-slate-800 placeholder:text-slate-400 min-w-0"
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder="Search products…"
                />
                ...
                {isAuthenticated ? (
                  ...
                  <div className="flex items-center gap-1.5">
                    <Link
                      href="/auth/login"
                      className="text-[13px] font-medium text-slate-600 hover:text-slate-900
                        px-3.5 py-2 rounded-lg hover:bg-slate-50 transition whitespace-nowrap"
                    >
                      Log in
                    </Link>
                    <Link
                      href="/auth/register"
                      className="text-[13px] font-medium px-4 py-2 bg-indigo-600
                        hover:bg-indigo-700 text-white rounded-lg transition whitespace-nowrap"
                    >
                      Register
                    </Link>
                  </div>
          ...
          <div className="flex items-center gap-3">
            {isAuthenticated ? (
              ...
              <>
                ...
                <div>
                  <p className="text-[13px] font-semibold text-slate-800">Welcome</p>
          ...
          {!loading && (
            <>
              ...
              {isAuthenticated ? (
                  ...
                  <div className="space-y-0.5">
                    {ACCOUNT_LINKS.map(({ label, href }) => (
                      ...
                      >
                        ...
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="ml-auto text-slate-300">
                          <polyline points="9 18 15 12 9 6" />
                        </svg>
                      </Link>
                    ))}
                ...
                <div className="space-y-2.5">
                  <Link
                    href="/auth/login" onClick={() => setMenuOpen(false)}
                    className="block text-center py-2.5 rounded-xl border border-slate-200
                      text-[13px] font-medium text-slate-700 hover:bg-slate-50 transition"
                  >
                    Log in
                  ...
                  <Link
                    href="/auth/register" onClick={() => setMenuOpen(false)}
                    className="block text-center py-2.5 rounded-xl bg-indigo-600
                      text-[13px] font-medium text-white hover:bg-indigo-700 transition"
                  >
                    Register
